...
void projection_T00_project(Particles<part, part_info, part_dataType> * pcls, Field<Real> * T00, double a = 1., Field<Real> * phi = NULL, double coeff = 1.)
{
	...
	for (xPart.first(),xField.first(); xPart.test(); xPart.next(),xField.next())
	{
		...
		{
		  for(int i=0; i<3; i++) referPos[i] = xPart.coord(i)*dx; //((int)(floor(rsd_pos[i]/dx))) % pcls->lattice().size(i);
...
template<typename part, typename part_info, typename part_dataType>
//------garbage
void projection_T00_project_rsdgarbage(Particles<part, part_info, part_dataType> * pcls, const cosmology cosmo, Field<Real> * T00, double fourpiG, double a = 1., Field<Real> * phi = NULL, double coeff = 1.)//change:passed csomlogy params
{
	if (T00->lattice().halo() == 0)
	{
		cout<< "projection_T00_project: target field needs halo > 0" << endl;
